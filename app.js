...
function renderSubjects() {
  ...
  list.innerHTML = state.subjects.map(subject => `
    <div class="subject-item ${state.activeSubject === subject.id ? 'active' : ''}" data-id="${subject.id}">
      ...
      <span class="subject-time">${formatTime(subject.totalTime)}</span>
...
function updateTotalTime() {
  const total = state.subjects.reduce((sum, s) => sum + s.totalTime, 0);
...
document.getElementById('addSubjectBtn').addEventListener('click', () => {
  ...
  if (existing) {
    ...
    const newSubject = { id: generateId(), name, totalTime: 0 };
    state.subjects.push(newSubject);
    state.activeSubject = newSubject.id;
  }
...
function updateTimerDisplay() {
  ...
  if (subject) {
    ...
    const currentTime = subject.totalTime + (state.isRunning ? state.timerSeconds : 0);
...
function toggleTimer() {
  ...
  if (state.isRunning) {
    ...
    const subject = state.subjects.find(s => s.id === state.activeSubject);
    if (subject) {
      subject.totalTime += state.timerSeconds;
